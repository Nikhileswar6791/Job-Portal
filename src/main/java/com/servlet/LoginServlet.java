		...
		try {
			...
			if("admin@gmail.com".equals(email) && "password".equals(password)) {
				...
				if(user !=null) {
					...
					CompanyDao dao1 = new CompanyDao(DBConnect.getConnction());
					company comp = dao1.validateCompany(email,password);
					if(comp != null)
					{
					  session.setAttribute("compobj", comp);
					  session.setAttribute("userobj", comp);
					  resp.sendRedirect("index.jsp");
					}
					else {
